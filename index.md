---
layout: default
title: Legal
permalink: /
---

# Vibely — Legal

Vibely is a mobile app for discovering and hosting local events. These are the documents
that govern it.

| Document | What it covers |
|---|---|
| [Privacy Policy]({{ '/privacy-policy/' | relative_url }}) | What the app collects, why, who it goes to, and how to get rid of it |
| [Terms of Service]({{ '/terms-of-service/' | relative_url }}) | The agreement you accept by using Vibely, including our content and safety rules |
| [Delete Your Account]({{ '/account-deletion/' | relative_url }}) | How to delete your account and data, in the app or by email |
| [Support]({{ '/support/' | relative_url }}) | Contact, reporting, safety guidance, and common questions |

**Effective 30 July 2026.**

---

## Contact

Vibely is operated by **Aryan Mehta**, an individual based in New Delhi, India.

For privacy requests, account deletion, safety reports, or anything else:
**[spacevibely@gmail.com](mailto:spacevibely@gmail.com)**
